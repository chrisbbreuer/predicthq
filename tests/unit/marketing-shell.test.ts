...
describe('shared marketing shell', () => {
  ...
  it('keeps the prediction-market loop and live desk on their realtime paths', async () => {
    ...
    expect(markets).toContain('setInterval(() => this.refreshFlow(), 60000)')
    expect(markets).toContain('init() {')
    expect(markets).toContain("predicthqRealtime.connect('{{ broadcastPort }}', 'prediction-markets'")
    expect(markets).toContain('onConnected: () => { this.connected = true }')
  ...
  it('uses the public TLS websocket and ts-broadcasting protocol', async () => {
    ...
    expect(realtime).toContain('useWebSocket(this.url(port)')
    expect(realtime).toContain("event: 'subscribe'")
    expect(live).toContain("predicthqRealtime.connect('{{ broadcastPort }}', ['odds', 'alerts']")
    expect(home).toContain("predicthqRealtime.connect('{{ broadcastPort }}', 'odds'")
